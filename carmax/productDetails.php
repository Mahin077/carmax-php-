...
if ($tbl== 'normal') {
    ...
    if (isset($_POST['submit_car'])) {
        if (!isset($_SESSION['user_id'])) {
            ...
            $sqlInsert = "insert into order_cars (user_id,email,phone,car_id,car_name,car_price,time) values('$uId','$uEmail',' $uPhone','$id','$carName','$carPrice','$time')";
            $resultIns = mysqli_query($conn, $sqlInsert);
            if ($resultIns) {
                echo "<script>alert('Thanks for being with us. We will contact with you soon!')</script>";
            } else {
...
else if($tbl == 'private' && isset($_SESSION['user_id']))
{
    ...
    echo "<script>alert('Please login first to view this page!');location.href='login.php';</script";
}
